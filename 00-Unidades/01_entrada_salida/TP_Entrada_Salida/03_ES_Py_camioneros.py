...
class App(customtkinter.CTk):
    ...
    def btn_cantidad_camiones_on_click(self):      #3.5 ton = 1 camion
        txt_toneladas = self.txt_toneladas.get()   
        toneladas = float(txt_toneladas)
        kg_camion = float(3.5)
        camiones = toneladas / kg_camion
        alert("cantidad camiones", camiones)

    def btn_tiempo_llegada_on_click(self):                #90 km/h cada camion
        txt_kilometros = self.txt_kilometros.get()         
        kilometros = float(txt_kilometros)
        velocidad = int(90)
        horas = kilometros / velocidad
        alert("horas estimadas", horas)
